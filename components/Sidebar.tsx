...
export function Sidebar({ nav }: { nav: NavSection[] }) {
  ...
  const { solved, revisit, toggle, toggleRevisit } = useProgress()
                ...
                if (item.href.startsWith('/dsa')) {
                  for (const phase of Object.values(DSA_DATA)) {
                    ...
                    if (topic) {
                      ...
                      solvedCount = topic.questions.filter(q => solved.has(q.id)).length
                      
                      // Pull questions if topic is expanded
                      if (isExpanded) {
                        let qs = topic.questions.map((q, originalIdx) => getEnrichedQuestion({ ...q, globalIdx: originalIdx + 1 }))
                        ...
                        const groups: Record<string, { id: number; globalIdx: number; label: string; href: string; companies: string[]; frequency: number; acRate: number; importance: string; difficulty: number; leetcodeNumber: number; platform: string; link?: string; questionSlug?: string }[]> = {}
                        qs.forEach((q) => {
